...
public final class CuboidJob extends Job {
    ...
    protected void executeTask(final int taskSize) {
        ...
        //save only if task is cancelled getAction().saveResults(getId());
    ...
    private boolean handleChunk(int chunkX, int chunkZ) {
        //direct Chunk chunk = world.getChunkAt(chunkX, chunkZ);
        ...
        for (int x = currentBlock.getBlockX(); x < 16; x++) {
            for (int z = startZ; z < 16; z++) {
                for (int y = startY; y <= maxCorner.getBlockY(); y++) {
                    if (taskCancelled) {
                        //DBUtil.logBlock(this, chunkX, chunkZ, x, y, z, getAction().getProcessedBlocks(),
                        //        getAction().getFoundBlocks());
                        currentBlock = new Vector(x, y, z);
                        return false;
                    }
                    //direct action.execute(chunk.getBlockAt(x,y,z));
                    //action.execute(chunk, x, y, z,
                    //        snap.getBlockTypeId(x, y, z),
                    //        snap.getBlockData(x, y, z));
                }
